...
- **Our frames vs. python:** SDO expedited download (U32/U8/I16), upload
  ...
  switch, store, inquire-node-id); and SDO block transfer both directions
  (download + upload initiate / start / sub-block / end command bytes, and the
  CRC-16/XMODEM against `binascii.crc_hqx`).
...
All 33 checks currently pass. This validates the **wire format**; it does not
